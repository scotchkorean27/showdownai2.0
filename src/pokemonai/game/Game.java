...
public class Game {
	...
	public ArrayList<GameEvent> useMove(BattlePokemon user, BattlePokemon target, Move move) {
		...
		if (!move.category().equals(MoveCategory.STATUS)) {
			...
			double totalacc = (double) move.accuracy() * user.getStatMult(BattleStat.ACCURACY) / target.getStatMult(BattleStat.EVASION);
	 ...
	 * Other: Add as needed
	 */
	public DamageEvent getModifiedDamage(BattlePokemon attacker, BattlePokemon defender, Move move, boolean isCrit, Random rand) {
		int damage = (int) (Math.floor(getBaseDamage(attacker, defender, move, isCrit))
				* (isCrit ? 1.5 : 1.0) 
		...
		return new DamageEvent(defender, true, isCrit, Effectiveness.getEffectiveness(typemult), (int) (damage * stab * typemult), false);
	}
