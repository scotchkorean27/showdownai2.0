	...
	public Integer getStat(Stat stat) {
		if(stat.equals(Stat.HP)) {
			return (int) ((2 * this.dexData.baseStats.get(Stat.HP) 
					...
					+ ((this.evs.containsKey(Stat.HP) ? this.evs.get(Stat.HP) : 0) / 4 )) 
					* this.level / 100) 
					+ this.level + 10;
		}
		else {
			return (int) ((Math.floor((2 * this.dexData.baseStats.get(stat) 
					+ (this.ivs.containsKey(stat) ? this.ivs.get(stat) : 0) 
					+ ((this.evs.containsKey(stat) ? this.evs.get(stat) : 0) / 4))
					* level / 100) + 5)
					* (this.nature.getUp().equals(Optional.of(stat)) ? 1.1 : 1.0)
